...
class Model:
    ...
    def create_tables(self):
        try:
            with sqlite3.connect(self.db_path) as con:
                ...
                # cur.execute('DROP TABLE IF EXISTS engineers')
                ...
                # cur.execute('DROP TABLE IF EXISTS Project_Type')
    ...
    def create_agents_db(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as con:
                ...
                # cur.execute('DROP TABLE IF EXISTS report_details')
                ...
                cur.execute('''
                    CREATE TABLE IF NOT EXISTS report_details (
                        ...
                        FOREIGN KEY (project_id) REFERENCES scan(id) ON DELETE CASCADE
                    )
                ''')
            ...
            with sqlite3.connect(db_path) as con:
                ...
                for engineer in engineer_names:
                    ...
                    for single_engineer in engineers_list:
                        ...
                        # Check if the engineer already exists
                        cur.execute('''
                            SELECT projects FROM engineers WHERE employee_name = ?
                        ''', (single_engineer,))
                        current_projects = cur.fetchone()
                        
                        if current_projects is not None:
                            # If the engineer exists, append the new project without an extra comma at the end
                            current_project_list = current_projects[0].split(", ") if current_projects[0] else []
                            if project_details[0] not in current_project_list:
                                current_project_list.append(project_details[0])

                            # Join the list back to a single string with commas between, but no trailing comma
                            updated_projects = ", ".join(current_project_list)
                            ...
                            cur.execute('''
                                ...
                                SET projects = ?
                                WHERE employee_name = ?
                            ''', (updated_projects, single_engineer))
                        else:
    ...
    def update_project(self, project_details, engineer_names):
        try:
            with sqlite3.connect(self.db_path) as con:
                ...
                # Fetch the current project name before updating
                cur.execute('''
                ...
                # Get the project_type_id based on project_type
                project_type = project_details[3]
                cur.execute('''
                    SELECT id FROM Project_Type WHERE type_name = ?
                ''', (project_type,))
                project_type_id = cur.fetchone()
                ...
                # Convert engineer names into a set, trimming whitespace
                new_engineers = {engineer.strip() for engineer in engineer_names[0].split(',')}

                # Fetch engineers currently associated with this project
                cur.execute('''
                    SELECT employee_name FROM engineers WHERE projects LIKE ?
                ''', (f'%{old_project_name}%',))
                existing_engineers = {row[0] for row in cur.fetchall()}

                # Determine engineers to update (new engineers + existing engineers)
                engineers_to_update = existing_engineers | new_engineers

                # Remove old project name and replace it with the new project name for relevant engineers
                for engineer in engineers_to_update:
                    # Fetch current projects for this engineer
                    cur.execute('''
                        SELECT projects FROM engineers WHERE employee_name = ?
                    ''', (engineer,))
                    result = cur.fetchone()
                    ...
                    # Remove old project name and replace it with new project name
                    updated_project_list = [
                            project_details[0] if p == old_project_name else p
                        for p in current_projects.split(", ") if p
                    ]

                    # Join the list back to a single string with commas
                    updated_projects = ", ".join(updated_project_list)

                    # Update the engineer's project list
                    cur.execute('''
                        UPDATE engineers
                        SET projects = ?
                        WHERE employee_name = ?
                    ''', (updated_projects, engineer))
    ...
    def delete_project(self, db_path, project_name):
        try:
            with sqlite3.connect(db_path) as con:
                ...
                # Step 1: Remove the project from engineers' project lists
                cur.execute('''
                    SELECT employee_name, projects FROM engineers
                ''')
                engineers = cur.fetchall()

                for engineer, current_projects in engineers:
                    # Only proceed if the project is part of the engineer's list
                    if project_name in current_projects:
                        # Remove the project name from the list
                        updated_project_list = [
                            p for p in current_projects.split(", ") if p != project_name
                        ]
                        updated_projects = ", ".join(updated_project_list)
                ...
                # Step 2: Delete associated SSH credentials
                cur.execute('''
                ...
                con.commit()
                return {"message": "Project and associated data deleted successfully"}, 200
            ...
            with sqlite3.connect(self.db_path) as con:
                ...
                cur.execute('''
                    SELECT 
                        ...
                        scan.current_user,
                        scan_data.scan_id,
                        projects.dut,           -- Add project_dut (DUT)
                        projects.project_type   -- Add project_type
                    FROM scan_data
                    JOIN scan ON scan_data.scan_id = scan.id
                    JOIN projects ON scan.project_id = projects.id
                ...
                uploads = cur.fetchall()
                uploads.reverse()  # Optional: Reverse the list order
    ...
    def delete_report(self, scan_id):
            try:
                with sqlite3.connect(self.db_path) as con:
                    ...
                    # Delete the record from the scan table (cascade deletes in report_details)
    ...
    def testcase_name_check(self, folder_name):
        """Check if the given folder name exists in the Project_Type table."""
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                cur.execute("SELECT type_name FROM Project_Type WHERE type_name = ?", (folder_name,))
                result = cur.fetchone()
            
                # If result is not None, folder_name exists in the table
                return result is not None
        except sqlite3.Error as e:
            print(f"An error occurred while fetching project type name data: {e}")
            return None

    def check_subtype_exists(self, project_type_name, subtype_name):
        """Check if the subtype belongs to the specified project type."""
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                
                # Join Project_Type and subtypes tables to check the relationship
                query = '''
                    SELECT subtypes.subtype_name
                    FROM subtypes
                    JOIN Project_Type ON subtypes.Project_Type_id = Project_Type.id
                    WHERE Project_Type.type_name = ? AND subtypes.subtype_name = ?
                '''
                cur.execute(query, (project_type_name, subtype_name))
                result = cur.fetchone()
    ...
    def insert_EUTConfiguration(self, current_user, model_no, product_name, manufacturer, serial_no,
        ...
        try:
            with sqlite3.connect(self.db_path) as con:
                ...
                # Check for duplicate product number before insertion
                cur.execute("SELECT COUNT(*) FROM EUTConfiguration WHERE product_no = ?", (product_no,))
                count_result = cur.fetchone()

                if count_result[0] > 0:
                    raise sqlite3.IntegrityError("Duplicate product number found.")

                # Insert data if product number is unique
                cur.execute('''
            ...
            return {"status": "error", "message": "Failed to create DUT Configuration: Duplicate product number found."}

        except sqlite3.Error as e:
            return {"status": "error", "message": "Failed to create DUT Configuration: Failed to create DUT Configuration: Database error."}
    ...
    def add_report_details(self, project_id, report_details):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                ...
                for detail in report_details:
                    # Check if the detail key already exists for the given project_id
                    cur.execute('''
                        SELECT COUNT(*) FROM report_details 
                        WHERE project_id = ? AND detail_key = ?
                    ''', (project_id, detail['key']))
                    (exists,) = cur.fetchone()
                    
                    if not exists:  # Only insert if the key does not exist
                        cur.execute('''
                            INSERT INTO report_details (project_id, detail_key, detail_value)
                            VALUES (?, ?, ?)
                        ''', (project_id, detail['key'], detail['value']))
                
                con.commit()  # Commit after all inserts
            return {"status": "success", "message": "Details added successfully."}
        
        except sqlite3.Error as e:
            print(f"Error adding report for project {project_id}: {e}")
            return {"status": "error", "message": "Failed to add report."}

    def add_json_report_details(self, project_id, report_details):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()

                # Check if report_details is a dictionary (flat key-value pairs)
                if isinstance(report_details, dict):
                    for key, value in report_details.items():
                        # Check if the key already exists for the given project_id
                        cur.execute('''
                            SELECT COUNT(*) FROM report_details 
                            WHERE project_id = ? AND detail_key = ?
                        ''', (project_id, key))
                ...
                # If report_details is a list of dictionaries, proceed with original structure
                elif isinstance(report_details, list):
                    for detail in report_details:
                        # Check if the detail key already exists for the given project_id
                        cur.execute('''
                            SELECT COUNT(*) FROM report_details 
    ...
    def get_report_details(self, project_id):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                cur.execute('''
                ...
                report_details = [{'key': row[0], 'value': row[1]} for row in rows]

            return {"status": "success", "data": report_details}
        except sqlite3.Error as e:
            print(f"Error retrieving report details for project {project_id}: {e}")
            return {"status": "error", "message": "Failed to retrieve report details."}


    def get_report_agent_details(self, project_name):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                ...
                project_id = project_row[0]  # Extract project_id

                # Fetch report details based on project_id
                cur.execute('''
                    SELECT detail_key, detail_value 
                    FROM report_details 
                    WHERE project_id = ?
            ...
            print(f"Error retrieving report details for project {project_name}: {e}")
            return {"status": "error", "message": "Failed to retrieve report details."}

    def add_type_details(self, type_name, subtypes):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                ...
                # Retrieve the Project_Type_id for the given type_name
                cur.execute('SELECT id FROM Project_Type WHERE type_name = ?', (type_name,))
                project_type_id = cur.fetchone()
                print("project_type_id:",project_type_id)

                if not project_type_id:
                    return {"status": "error", "message": "Project type not found."}
                
                project_type_id = project_type_id[0]  # Get the ID value from the tuple
                print("project_type_id:",project_type_id)
                # Insert each subtype with the corresponding Project_Type_id
                for subtype in subtypes:
                    ...
                    WHERE subtype_name = ? AND Project_Type_id = ?
                    ''', (subtype_name, project_type_id))
                    (exists,) = cur.fetchone()
                    
                    if not exists:  # Only insert if the subtype does not exist
                        cur.execute('''
                            INSERT INTO subtypes (subtype_name, Project_Type_id) 
                            VALUES (?, ?)
                        ''', (subtype_name, project_type_id))
            
                con.commit()
                return {"status": "success", "message": "Subtypes added successfully."}
        
        except sqlite3.Error as e:
            print(f"Error adding subtypes for type {type_name}: {e}")
            return {"status": "error", "message": "Failed to add subtypes."}


    
    def add_json_type_details(self, type_name, subtypes):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                ...
                # Process subtypes based on their structure
                if isinstance(subtypes, dict):
                    ...
                    for detail in subtypes:
                        subtype_name = detail.get('subtype_name')
                        if subtype_name:
                            cur.execute('''
                                SELECT COUNT(*) FROM subtypes 
                                WHERE subtype_name = ? AND Project_Type_id = ?
            ...
            print(f"Error adding report for project {project_type_id}: {e}")
            return {"status": "error", "message": "Failed to add report."}
        
    # def delete_type_details(self, type_name, subtype_name):
    #     try:
    #         with sqlite3.connect(self.db_path) as con:
    #             cur = con.cursor()
    ...
    #             project_type_id = project_type_id[0]  # Extract the ID from the tuple

    #             # Delete the specified subtype
    #             cur.execute('''
    ...
    #             if cur.rowcount == 0:
    #                 return {"status": "error", "message": "Subtype not found."}

    #             con.commit()
    #             return {"status": "success", "message": "Subtype deleted successfully."}

    #     except sqlite3.Error as e:
    #         print(f"Error deleting subtype {subtype_name} for type {type_name}: {e}")
    #         return {"status": "error", "message": "Failed to delete subtype."}

    def delete_type_details(self, subtype_name):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                print("subtype_name model",subtype_name)
                # Delete the specified subtype
                cur.execute('''
                    DELETE FROM subtypes 
                    WHERE subtype_name = ?
            ...
            print(f"Error deleting subtype {subtype_name}: {e}")
            return {"status": "error", "message": "Failed to delete subtype."}

    def update_subtype_details(self, old_subtype_name, new_subtype_name):
        try:
            with sqlite3.connect(self.db_path) as con:
                cur = con.cursor()
                print("old_subtype_name:", old_subtype_name, "new_subtype_name:", new_subtype_name)
                ...
                cur.execute('''
                    ...
                    SET subtype_name = ? 
                    WHERE subtype_name = ?
                ''', (new_subtype_name, old_subtype_name))

                if cur.rowcount == 0:
                    return {"status": "error", "message": "Subtype not found."}

                con.commit()
                return {"status": "success", "message": "Subtype updated successfully."}

        except sqlite3.Error as e:
            print(f"Error updating subtype {old_subtype_name} to {new_subtype_name}: {e}")
            return {"status": "error", "message": "Failed to update subtype."}
    ...
    def delete_item(self, key):
        try:
            with sqlite3.connect(self.db_path) as con:
                print("2291")
                cur = con.cursor()
                # Delete the record from the report_details table
                cur.execute('DELETE FROM report_details WHERE detail_key = ?', (key,))
                con.commit()

                # Check if the deletion affected any rows (if item_id existed)
                if cur.rowcount == 0:
