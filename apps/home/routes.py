...
def delete_item():
    ...
    # Get the key from the received data
    key = data.get('key')
    if not key:
        return jsonify({"status": "error", "message": "Invalid Key"}), 400

    try:
        # Interact with the Model to delete the item using the key
        Model_obj = Model()
        delete_result = Model_obj.delete_item(key)  # Assuming you have a method to delete by key
    ...
    if request.method == 'POST':
        ...
        if type_req == "eut_conf":
            try:      
                ...
                response = Model_obj.insert_EUTConfiguration(
...
@blueprint.route('/view_subtype_report_files', methods=['POST', 'GET'])
@login_required
def view_subtype_report_files():
    if request.method == 'POST':
        ...
        if not project_type or not project_name or not project_dut or not subtype_name or not scan_id:
            return "All parameters post(project_type, project_name, project_dut, subtype_name, scan_id) are required.", 400
        ...
        try:
            ...
            # Set flag for showing the table if there are no images
            show_table = not bool(images)
        except FileNotFoundError:
            files = []
            images = []
            image_basenames = []  # Ensure this is empty on error
        ...
        # Redirect to the same route with the query parameters to avoid re-submission on refresh
        return redirect(url_for('home_blueprint.view_subtype_report_files', 
    ...
    project_dut = request.args.get('project_dut')
    subtype_name = request.args.get('subtype_name')
    scan_id = request.args.get('scan_id')
    print("get methoad")
    if not project_type or not project_name or not project_dut or not subtype_name or not scan_id:
        return "All parameters (project_type, project_name, project_dut, subtype_name, scan_id) are required.", 400
    ...
    # Render the template with the separated file lists and show_table flag
    return render_template(
        'home/scan_report.html',
        files=files,
        images=images,
...
@blueprint.route('/testcase_name_check', methods=['POST'])
def testcase_name_check():
    """Check if the folder name exists in the Project_Type table."""
    data = request.json
    # print("data:", data)
    ...
    # Initialize the model and check the folder name
    model_obj = Model()
    exists = model_obj.testcase_name_check(folder_name)

    if exists:
        return jsonify({"exists": True}), 200
    else:
        return jsonify({"exists": False, "error": "Folder name not allowed"}), 403

@blueprint.route('/verify_zip_name', methods=['POST'])
def verify_zip_name():
    """Check if the provided project type has the specified subtype."""
    data = request.json
    project_type_name = data.get('folder_name')
    subtype_name = data.get('zip_name')
    
    # Check for valid input
    if not project_type_name or not subtype_name:
        return jsonify({"error": "Invalid input, folder_name or zip_name missing"}), 400
    
    # Initialize model
    model = Model()
    
    # Verify if subtype belongs to the project type
    exists = model.check_subtype_exists(project_type_name, subtype_name)
    
    # Return result
    if exists:
        return jsonify({"exists": True}), 200
    else:
        return jsonify({"exists": False, "error": "Subtype does not match the specified project type"}), 404
...
def create_scan_id():
    ...
    report_details = model_obj.get_report_agent_details(project)
    print("report_details:",report_details)
    if report_details["status"] == "error":
        return jsonify({"error": report_details["message"]}), 400

    # Return the response with scan_id, repo_dir, SSH credentials, project type, DUT details, and report details
    return jsonify({
        "scan_id": scan_id,
        "repo_dir": repo_dir,
        ...
        'report_details': report_details["data"]  # Return report details
    }), 200
...
def view_subtypes():
    ...
    if request.method == 'POST':
        ...
        # Redirect to the same page with the form data as query parameters
        return redirect(url_for('home_blueprint.view_subtypes', 
                                project_name=project_name, 
                                project_type=project_type, 
                                project_dut=project_dut))

    # For GET requests, retrieve data from URL query parameters
    project_name = request.args.get('project_name')
    project_type = request.args.get('project_type')
    project_dut = request.args.get('project_dut')

    if not project_name or not project_type or not project_dut:
        return "Required parameters not found", 400

    # Remaining logic to retrieve and prepare data for rendering
    model_obj = Model()
    ...
    report_details = model_obj.get_report_details(project_id)
    # print("report_details:",report_details)
...
@blueprint.route('/view_all_subtype', methods=['GET', 'POST'])
def view_all_subtype():
    if request.method == 'POST':
        ...
        # Redirect to the same page with the form data as query parameters
        return redirect(url_for('home_blueprint.view_all_subtype', 
                                project_name=project_name, 
                                project_type=project_type, 
                                project_dut=project_dut))
    ...
    # if not subtypes:
    #     return "No subtypes found for this project type", 404
    
    # Add status to each subtype
    for subtype in subtypes:
        subtype['status'] = model_obj.get_status_by_subtype(subtype['subtype_name'], project_name)
    
    return render_template('home/all_subtype.html', 
                           subtypes=subtypes, 
                           project_name=project_name, 
                           project_type=project_type,
                           project_dut=project_dut)
...
@blueprint.route('/add_type', methods=['GET', 'POST'])
@login_required
def add_type():
    model_obj = Model()
    ...
    if request.method == 'POST':
        try:
            ...
            if request.is_json:
                ...
                project_dut = data.get('project_dut')
                subtypes = data.get('type_details')

                if not subtypes:
                    flash('No type details provided in JSON.', 'error')
                    return jsonify({'success': False, 'error': 'No type details provided'})

                # Save type details from JSON data
                model_obj.add_json_type_details(project_type, subtypes)
                flash('Type details added successfully!', 'success')
            ...
            print("subtypes:",subtypes)
            subtype_data = [{"subtype_name": name} for name in subtypes]


            if not subtype_data :
                flash('Please fill type details.', 'error')
                return redirect(url_for('home_blueprint.view_all_subtype',
                                        project_name=project_name,
                                        project_type=project_type,
                                        project_dut=project_dut))

            model_obj.add_type_details(project_type, subtype_data)
            flash('Types details added successfully!', 'success')
            return redirect(url_for('home_blueprint.view_all_subtype',
                                    project_name=project_name,
                                    project_type=project_type,
                                    project_dut=project_dut))
            ...
            flash(f'Error adding type details: {e}', 'error')
...
@blueprint.route('/delete_type', methods=['POST'])
@login_required
def delete_type():
    model_obj = Model()

    # If the request is form data instead of JSON
    if request.form:
        ...
        # Validate inputs
        if not subtype_name:
            return jsonify({'success': False, 'error': 'Project type and subtype name are required.'})

        # Call the model method to delete subtype details
        result = model_obj.delete_type_details(subtype_name)

        # Check the result and return appropriate responses
        ...
        flash('Invalid request format. Form data required.', 'error')
        return redirect(url_for('home_blueprint.view_all_subtype',
                                project_name=project_name,
                                project_type=project_type,
                                project_dut=project_dut))

@blueprint.route('/edit_subtype/<subtype_name>', methods=['POST'])
@login_required
def edit_subtype(subtype_name):
    model_obj = Model()
    ...
    project_type = request.form.get('project_type')
    project_dut = request.form.get('project_dut')
    new_subtype_name = request.form.get('subtype_name')  # This will be the new name for the subtype
    print("new_subtype_name:", new_subtype_name)

    # Validate inputs
    if not new_subtype_name:
        flash('Subtype name is required.', 'error')
        return redirect(url_for('home_blueprint.view_all_subtype',
                                project_name=project_name,
                                project_type=project_type,
                                project_dut=project_dut))

    # Call the model method to edit subtype details
    result = model_obj.update_subtype_details(subtype_name, new_subtype_name)
    ...
    # Redirect back to the view_all_subtype route
    return redirect(url_for('home_blueprint.view_all_subtype',
                            project_name=project_name,
                            project_type=project_type,
                            project_dut=project_dut))

@blueprint.route('/subtype-details', methods=['POST', 'GET'])
@login_required
def subtype_details():
    if request.method == 'POST':
        # Fetch form data from POST request
        ...
        # Validate required fields
        if not subtype_name or not project_name or not project_type or not project_dut:
            return "All parameters (subtype_name, project_name, project_type, project_dut) are required.", 400

        # Initialize model object
        model_obj = Model()

        # Fetch subtype details from the database using the model
        subtype_details = model_obj.get_subtype_details_from_db(subtype_name, project_name)

        # if not subtype_details:
        #     return f"No details found for subtype: {subtype_name} in project: {project_name}", 404

        # Redirect to the GET request version of this route, passing parameters in the URL
        return redirect(url_for('home_blueprint.subtype_details', 
                                project_name=project_name, 
                                project_type=project_type, 
                                project_dut=project_dut, 
                                subtype_name=subtype_name))
    ...
    return render_template('home/subtype_details.html', 
                           uploads=subtype_details, 
                           project_name=project_name, 
                           project_type=project_type, 
                           project_dut=project_dut, 
                           subtype_name=subtype_name)
...
@blueprint.route('/add_report/<int:project_id>', methods=['GET', 'POST'])
@login_required
        ...
        try:
            ...
            if request.is_json:
                ...
                if not report_details:
                    flash('No report details provided in JSON.', 'error')
                    return jsonify({'success': False, 'error': 'No report details provided'})

                # Save report details from JSON data
                model_obj.add_json_report_details(project_id, report_details)
                flash('Report details added successfully!', 'success')
                return jsonify({'success': True})
            ...
            project_type = request.form.get('project_type')
            project_dut = request.form.get('project_dut')
            detail_keys = request.form.getlist('detail_key[]')
            detail_values = request.form.getlist('detail_value[]')

            if not detail_keys or not detail_values or len(detail_keys) != len(detail_values):
                flash('Please fill out all report details.', 'error')
                return redirect(url_for('home_blueprint.view_subtypes',
                                        project_name=project_name,
                                        project_type=project_type,
                                        project_dut=project_dut))
            ...
            report_details = [{"key": key, "value": value} for key, value in zip(detail_keys, detail_values)]
            
            model_obj.add_report_details(project_id, report_details)
            flash('Report details added successfully!', 'success')
            return redirect(url_for('home_blueprint.view_subtypes',
                                    project_name=project_name,
                                    project_type=project_type,
            ...
            flash(f'Error adding report details: {e}', 'error')
            return jsonify({'success': False, 'error': str(e)})
